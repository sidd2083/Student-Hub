...
import {
  ...
  getListUsersQueryKey, getListNotesQueryKey, getListMcqsQueryKey, getListPyqsQueryKey,
  getListScoresQueryKey, getGetUserStatsQueryKey
} from "@workspace/api-client-react";
import { useQueryClient } from "@tanstack/react-query";
import {
  LayoutDashboard, BookOpen, Brain, FileText, Users, Trophy,
  Shield, Plus, Trash2, LogOut
} from "lucide-react";

type Section = "dashboard" | "notes" | "mcqs" | "pyqs" | "users" | "leaderboard";
...
function ManageNotes() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    createNote.mutate({ data: form }, { onSuccess: () => { setShowForm(false); setForm({ grade: 10, subject: "", chapter: "", title: "", contentType: "text", content: "" }); inv(); } });
  };
    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6 space-y-4">
          ...
          {["subject", "chapter", "title"].map(f => (
            <div key={f}>
              <label className="block text-xs font-medium text-gray-600 mb-1 capitalize">{f}</label>
              <input value={(form as Record<string,string|number>)[f] as string} onChange={e => setForm(prev => ({ ...prev, [f]: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm" required />
            </div>
          ))}
          <div>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={createNote.isPending} className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm hover:bg-blue-600 transition-all">Save</button>
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm hover:bg-gray-200 transition-all">Cancel</button>
          </div>
        </form>
      )}
      {isLoading ? <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-14 bg-gray-100 rounded-xl animate-pulse" />)}</div>
        : (notes || []).map(n => (
          <div key={n.id} className="flex items-center justify-between bg-white rounded-xl border border-gray-100 px-4 py-3 mb-2">
            ...
            <button onClick={() => deleteNote.mutate({ id: n.id }, { onSuccess: inv })} className="p-1.5 text-gray-400 hover:text-red-500 transition-all">
...
function ManageMcqs() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    createMcq.mutate({ data: form }, { onSuccess: () => { setShowForm(false); inv(); } });
  };
    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6 space-y-4">
          ...
          {["subject","chapter","question","optionA","optionB","optionC","optionD"].map(f => (
            <div key={f}>
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={createMcq.isPending} className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm">Save</button>
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm">Cancel</button>
          </div>
        </form>
      )}
      {isLoading ? <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-14 bg-gray-100 rounded-xl animate-pulse" />)}</div>
        : (mcqs || []).map(m => (
          <div key={m.id} className="flex items-center justify-between bg-white rounded-xl border border-gray-100 px-4 py-3 mb-2">
            ...
            <button onClick={() => deleteMcq.mutate({ id: m.id }, { onSuccess: inv })} className="p-1.5 text-gray-400 hover:text-red-500 transition-all">
              <Trash2 className="w-4 h-4" />
            </button>
          </div>
        ))}
...
function ManagePyqs() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    createPyq.mutate({ data: form }, { onSuccess: () => { setShowForm(false); inv(); } });
  };
    ...
    <div>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6 space-y-4">
          ...
          <div className="flex gap-2">
            <button type="submit" disabled={createPyq.isPending} className="px-4 py-2 bg-blue-500 text-white rounded-lg text-sm">Save</button>
            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 bg-gray-100 text-gray-600 rounded-lg text-sm">Cancel</button>
          </div>
        </form>
      )}
      {isLoading ? <div className="space-y-2">{[1,2,3].map(i => <div key={i} className="h-14 bg-gray-100 rounded-xl animate-pulse" />)}</div>
        : (pyqs || []).map(p => (
          <div key={p.id} className="flex items-center justify-between bg-white rounded-xl border border-gray-100 px-4 py-3 mb-2">
            ...
            <button onClick={() => deletePyq.mutate({ id: p.id }, { onSuccess: inv })} className="p-1.5 text-gray-400 hover:text-red-500 transition-all">
              <Trash2 className="w-4 h-4" />
            </button>
          </div>
        ))}
        ...
        : (users || []).map(u => (
          <div key={u.id} className="flex items-center justify-between bg-white rounded-xl border border-gray-100 px-4 py-3 mb-2">
            ...
            <div className="flex items-center gap-2">
              <select
                value={u.role}
                onChange={e => updateUser.mutate({ uid: u.uid, data: { role: e.target.value as "user"|"admin" } }, { onSuccess: inv })}
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <Link href="/dashboard"><a className="text-blue-500 hover:underline">Go to Dashboard</a></Link>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-screen bg-gray-50">
      <aside className="w-64 bg-white border-r border-gray-100 flex flex-col shadow-sm">
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map(({ key, icon: Icon, label }) => (
            <button key={key} onClick={() => setSection(key)}
              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all text-left ${section === key ? "bg-purple-50 text-purple-600" : "text-gray-600 hover:bg-gray-50"}`}>
        ...
        <div className="p-4 border-t border-gray-100">
          <Link href="/dashboard">
            <a className="flex items-center gap-2 px-3 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg transition-all mb-1">
              Back to App
            </a>
          </Link>
          <button onClick={signOut} className="flex items-center gap-2 px-3 py-2 text-sm text-gray-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-all w-full">
