      ...
      <div className="flex gap-2 mb-6">
        {(["daily", "alltime"] as const).map((p) => (
          <button key={p} data-testid={`period-${p}`} onClick={() => setPeriod(p)}
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${period === p ? "bg-blue-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          {(scores || []).map((s, i) => {
            ...
            return (
              <div key={s.id} data-testid={`score-row-${s.id}`}
