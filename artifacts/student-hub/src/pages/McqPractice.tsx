...
export default function McqPractice() {
    ...
    return (
      <>
        <Helmet>
          ...
          <meta name="description" content="Practice multiple-choice questions by grade and subject." />
        </Helmet>
        <Layout>
          <div className="p-8 max-w-2xl mx-auto">
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center">
              ...
              <p className="text-sm text-gray-400 mb-8">{pct >= 70 ? "Excellent work! Keep it up." : pct >= 40 ? "Good effort — review the topics you missed." : "Keep practising — you'll get there!"}</p>
  ...
  if (phase === "quiz") {
    const q = mcqs?.[currentIdx];
    if (!q) {
      return <Layout><div className="p-8 text-center text-gray-500"><Brain className="w-12 h-12 mx-auto mb-3 opacity-30" /><p>Loading questions…</p></div></Layout>;
    }
          ...
          <div className="p-8 max-w-2xl mx-auto">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="w-32 h-2 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-blue-500 rounded-full transition-all" style={{ width: `${((currentIdx + 1) / mcqs.length) * 100}%` }} /></div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-4">
              ...
              <div className="space-y-3">
                {opts.map(([key, text]) => {
                  ...
                  return (
                    <button key={key} data-testid={`option-${key}`} onClick={() => handleSelect(key)} className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl border-2 transition-all text-left ${cls}`}>
            ...
            <div className="flex gap-3 justify-end">
              {!revealed ? (
                <button data-testid="btn-confirm-answer" onClick={handleConfirm} disabled={!selected} className="px-6 py-2.5 bg-blue-500 text-white rounded-xl font-medium hover:bg-blue-600 transition-all disabled:opacity-50">Confirm Answer</button>
              ) : (
                <button data-testid="btn-next-question" onClick={handleNext} className="px-6 py-2.5 bg-blue-500 text-white rounded-xl font-medium hover:bg-blue-600 transition-all">Next Question</button>
              )}
            </div>
          </div>
        </Layout>
      ...
      <Layout>
        <div className="p-8 max-w-2xl mx-auto">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h1 className="text-2xl font-bold text-gray-900 mb-6">MCQ Practice</h1>
            <p className="text-gray-500 mb-6">Choose your subject, difficulty, and number of questions.</p>
            <button onClick={startQuiz} className="px-6 py-3 bg-blue-500 text-white rounded-xl font-medium hover:bg-blue-600 transition-all">Start Quiz</button>
          </div>
        </div>
      </Layout>
    </>
