...
function ZoomLightbox({ src, alt, onClose }: { src: string; alt: string; onClose: () => void }) {
  return (
    <div
      className="fixed inset-0 bg-black/95 z-[70] flex items-center justify-center cursor-zoom-out"
      onClick={onClose}
    >
...
function FocusReaderOverlay({
  note,
  onClose,
  onAskAi,
}: {
  note: { title: string; content: string; contentType: string; subject: string; chapter: string; grade: number };
  onClose: () => void;
  onAskAi: () => void;
}) {
